...
public class RedisCacheConfig {
    ...
    public RedisCacheConfiguration redisCacheConfiguration() {
        ...
        // 进行redis配置
        redisCacheConfiguration1.serializeValuesWith(RedisSerializationContext.SerializationPair.fromSerializer(RedisSerializer.json())) // 设置redis值的序列化方式为json格式
                .entryTtl(Duration.ofDays(7))   // 统一设置redis中值的默认过期时间（7天）
                .disableCachingNullValues();    // redis的value值禁止使用空值
        return redisCacheConfiguration1;
    }
}
